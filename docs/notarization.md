...
3. Label it `mixr notarization` (or similar). Copy the generated password.
...
```sh
gh secret set APPLE_TEAM_ID --repo chris-mclennan/mixr
gh secret set APPLE_DEVELOPER_ID_CERT_BASE64 --repo chris-mclennan/mixr
gh secret set APPLE_DEVELOPER_ID_CERT_PASSWORD --repo chris-mclennan/mixr
gh secret set APPLE_ID --repo chris-mclennan/mixr
...
spctl -a -t open --context context:primary-signature \
    -v "Downloads/mixr-rs-aarch64-apple-darwin.dmg"
# Expect: "Downloads/mixr-rs-aarch64-apple-darwin.dmg: accepted"

# And check the staple:
xcrun stapler validate "Downloads/mixr-rs-aarch64-apple-darwin.dmg"
