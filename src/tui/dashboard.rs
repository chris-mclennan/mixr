...
pub enum CtrlSection {
    ...
    TempoA, VolumeA, CueA, PlayA, JumpA, NudgeA,
    // Per-deck mixer (live in the integrated Controller+Mixer panel).
    EqLowA, EqMidA, EqHighA, FilterA,
    Crossfader,
    EqLowB, EqMidB, EqHighB, FilterB,
    NudgeB, JumpB, PlayB, CueB, VolumeB, TempoB,
}
...
impl CtrlSection {
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::TempoA => "tempo A",   Self::TempoB => "tempo B",
            Self::VolumeA => "volume A", Self::VolumeB => "volume B",
            Self::CueA => "cue A",       Self::CueB => "cue B",
            Self::PlayA => "play A",     Self::PlayB => "play B",
            Self::JumpA => "jump A",     Self::JumpB => "jump B",
            Self::NudgeA => "nudge A",   Self::NudgeB => "nudge B",
            Self::EqLowA => "EQ low A",  Self::EqLowB => "EQ low B",
            Self::EqMidA => "EQ mid A",  Self::EqMidB => "EQ mid B",
            Self::EqHighA => "EQ high A",Self::EqHighB => "EQ high B",
            Self::FilterA => "filter A", Self::FilterB => "filter B",
...
impl CtrlSection {
    ...
    pub fn next(self) -> Self {
        match self {
            Self::TempoA => Self::VolumeA, Self::VolumeA => Self::CueA,
            Self::CueA => Self::PlayA, Self::PlayA => Self::JumpA,
            Self::JumpA => Self::NudgeA, Self::NudgeA => Self::EqLowA,
            Self::EqLowA => Self::EqMidA, Self::EqMidA => Self::EqHighA,
            Self::EqHighA => Self::FilterA, Self::FilterA => Self::Crossfader,
            Self::Crossfader => Self::EqLowB, Self::EqLowB => Self::EqMidB,
            Self::EqMidB => Self::EqHighB, Self::EqHighB => Self::FilterB,
            Self::FilterB => Self::NudgeB, Self::NudgeB => Self::JumpB,
            Self::JumpB => Self::PlayB, Self::PlayB => Self::CueB,
            Self::CueB => Self::VolumeB, Self::VolumeB => Self::TempoB,
            Self::TempoB => Self::TempoA,
        }
    }
    pub fn prev(self) -> Self {
        match self {
            Self::TempoA => Self::TempoB, Self::VolumeA => Self::TempoA,
            Self::CueA => Self::VolumeA, Self::PlayA => Self::CueA,
            Self::JumpA => Self::PlayA, Self::NudgeA => Self::JumpA,
            Self::EqLowA => Self::NudgeA, Self::EqMidA => Self::EqLowA,
            Self::EqHighA => Self::EqMidA, Self::FilterA => Self::EqHighA,
            Self::Crossfader => Self::FilterA,
            Self::EqLowB => Self::Crossfader, Self::EqMidB => Self::EqLowB,
            Self::EqHighB => Self::EqMidB, Self::FilterB => Self::EqHighB,
            Self::NudgeB => Self::FilterB, Self::JumpB => Self::NudgeB,
            Self::PlayB => Self::JumpB, Self::CueB => Self::PlayB,
            Self::VolumeB => Self::CueB, Self::TempoB => Self::VolumeB,
        }
    }
    ...
    pub fn label(self) -> &'static str {
        match self {
            Self::TempoA => "TempoA", Self::VolumeA => "VolumeA", Self::CueA => "CueA",
            Self::PlayA => "PlayA", Self::JumpA => "JumpA", Self::NudgeA => "NudgeA",
            Self::EqLowA => "EqLowA", Self::EqMidA => "EqMidA",
            Self::EqHighA => "EqHighA", Self::FilterA => "FilterA",
            Self::Crossfader => "Crossfader",
            Self::EqLowB => "EqLowB", Self::EqMidB => "EqMidB",
            Self::EqHighB => "EqHighB", Self::FilterB => "FilterB",
            Self::NudgeB => "NudgeB", Self::JumpB => "JumpB", Self::PlayB => "PlayB",
            Self::CueB => "CueB", Self::VolumeB => "VolumeB", Self::TempoB => "TempoB",
        }
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum WaveformMode { Phrase, Audio, Off }
impl WaveformMode {
    pub fn next(self) -> Self { match self { Self::Phrase => Self::Audio, Self::Audio => Self::Off, Self::Off => Self::Phrase } }
    pub fn label(self) -> &'static str { match self { Self::Phrase => "phrase", Self::Audio => "audio", Self::Off => "off" } }
}

fn pc(ms: f64) -> Color { let a = ms.abs(); if a < 5.0 { Color::Green } else if a < 15.0 { Color::Yellow } else { Color::Red } }
...
fn ts(s: &str, w: usize) -> String { if w <= 2 { return String::new(); } if s.chars().count() > w { let t: String = s.chars().take(w-1).collect(); format!("{t}…") } else { s.to_string() } }
...
fn render_two_col_box<'a>(
    ...
    let bs = if focused { Style::default().fg(Color::Cyan) } else { Style::default().fg(Color::DarkGray) };
    ...
    out.push(Line::from(Span::styled(format!("┌─ {title} {}┐", "─".repeat(tr)), bs)));
    ...
    if items.is_empty() {
        ...
        for row in 0..half {
            ...
            let right = if row + half < n { Some(&items[row + half]) } else { None };
            let mut spans: Vec<Span> = vec![pipe_l()];
            spans.push(Span::styled("● ", Style::default().fg(lc)));
            spans.push(Span::styled(ps(lt, left_w), Style::default().fg(Color::Gray)));
            if let Some((rc, rt)) = right {
                spans.push(pipe_mid());
                spans.push(Span::styled("● ", Style::default().fg(*rc)));
                spans.push(Span::styled(ps(rt, right_w), Style::default().fg(Color::Gray)));
            } else {
                spans.push(pipe_mid());
    ...
    out.push(Line::from(Span::styled(format!("└{}┘", "─".repeat(bxw)), bs)));
}

fn camelot_dist(a: &str, b: &str) -> usize {
    fn p(k: &str) -> Option<(i32, u8)> { let k = k.trim(); let l = *k.as_bytes().last()?; if l != b'A' && l != b'B' { return None; } k[..k.len()-1].parse().ok().map(|n| (n, l)) }
    let (na, la) = match p(a) { Some(v) => v, None => return 99 };
    let (nb, lb) = match p(b) { Some(v) => v, None => return 99 };
    if la == lb { let d = (na-nb).unsigned_abs() as usize; d.min(12-d) }
    else { if na == nb { 1 } else { let d = (na-nb).unsigned_abs() as usize; d.min(12-d) + 1 } }
}

fn compat(cb: Option<f64>, ck: Option<&str>, nb: Option<f64>, nk: Option<&str>) -> (u32, Color) {
    let mut s = 0.0;
    if let (Some(c), Some(n)) = (cb, nb) { let r = c.max(n)/c.min(n); let h = if r > 1.8 { r/2.0 } else { r }; s += (50.0 - (h-1.0).abs()*500.0).max(0.0); }
    if let (Some(a), Some(b)) = (ck, nk) { let d = camelot_dist(a, b); s += match d { 0=>50.0, 1=>45.0, 2=>30.0, _=>(20.0-d as f64*5.0).max(0.0) }; }
    let p = (s as u32).min(100);
    (p, if p >= 80 { Color::Green } else if p >= 60 { Color::Yellow } else { Color::Red })
}
...
fn render_claude_dj_section<'a>(
    ...
    if dj_log.is_empty() && dj_ask.is_none() { return; }
...
fn render_queue_section<'a>(
    ...
    for idx in 0..sc {
        ...
        let bk = format!("{}/{}", t.bpm.map(|b| format!("{:.0}",b)).unwrap_or("?".into()), t.key.as_deref().unwrap_or("?"));
        ...
        pb = t.bpm; pk = t.key.clone();
    }
    let overflow = (info.queue.len() > sc).then(|| format!("… {} more", info.queue.len()-sc));
    render_two_col_box(out, &format!("QUEUE ({}) [q]", info.queue.len()), &items, overflow.as_deref(), "Empty", w, focused);
    out.push(Line::from(""));
}
...
fn render_history_section<'a>(
    ...
    if info.history.len() >= 2 {
        ...
        for i in 0..rec.len() {
            ...
            let pk = if i > 0 { rec[i-1].track.key.as_deref() } else { None };
            let (pct, clr) = compat(pb, pk, e.track.bpm, e.track.key.as_deref());
            let bk = format!("{}/{}", e.track.bpm.map(|b|format!("{:.0}",b)).unwrap_or("?".into()), e.track.key.as_deref().unwrap_or("?"));
            let mix = e.mix_score.map(|s| format!(" m{s}")).unwrap_or_default();
            let nw = cw.saturating_sub(26);
            let nm = ts(&format!("{} - {}", e.track.artist_name(), e.track.full_title()), nw);
    ...
    let sparkline: String = if bpms.len() >= 3 {
        ...
        let rng = mx - mn;
        let trend: String = bpms.iter().map(|b| {
            let i = ((b - mn) / rng * (spark.len() - 1) as f64) as usize;
            spark[i.min(spark.len() - 1)]
        }).collect();
        format!(" {trend}")
    } else { Default::default() };
    let session: String = if info.session_time_min > 0 { format!(" {}m", info.session_time_min) } else { Default::default() };
    render_two_col_box(out, &format!("HISTORY ({}{sparkline}{session}) [h]", info.history.len()), &items, None, "No history yet", w, focused);
    out.push(Line::from(""));
}
...
fn render_browse_section<'a>(
    ...
    if browse_items.is_empty() { return; }
    ...
    for i in 0..item_count {
        click_targets.push(crate::tui::app::ClickTarget::new(
            area.x + 1, box_top_y + 2 + i as u16, w.saturating_sub(2) as u16, 1,
    ...
    out.push(Line::from(Span::styled(format!("┌─ {title} {}┐", "─".repeat(tr)), browse_bs)));
    out.push(Line::from(vec![
        Span::styled("│ ".to_string(), browse_bs),
        Span::styled(ps(browse_breadcrumb, cw), Style::default().fg(Color::DarkGray)),
        Span::styled(" │".to_string(), browse_bs),
    ]));
    ...
    out.push(Line::from(Span::styled(format!("└{}┘", "─".repeat(bw)), browse_bs)));
    out.push(Line::from(""));
}
...
fn render_log_section<'a>(
    ...
    out.push(Line::from(Span::styled(format!("┌─ {log_title} {}┐", "─".repeat(log_tr)), log_bs)));
    for line in ll.iter() {
        out.push(Line::from(vec![
            Span::styled("│ ".to_string(), log_bs),
            Span::styled(ps(line, log_cw).to_string(), Style::default().fg(Color::Gray)),
            Span::styled(" │".to_string(), log_bs),
        ]));
    }
...
pub fn render_dashboard(frame: &mut Frame, area: Rect, info: &NowPlayingInfo, wf_mode: WaveformMode, browse_items: &[String], browse_breadcrumb: &str, browse_selected: usize, browse_is_tracks: bool, show_help: bool, sel_section: Option<CtrlSection>, download_in_flight: bool, dj_log: &[String], dj_ask: Option<&str>, click_targets: &mut Vec<crate::tui::app::ClickTarget>, dash_focus: crate::tui::app::DashFocus, log_scroll_offset: usize, waveform_zoom: Option<bool>, dash_layout: DashLayout, panel_section: PanelSection) {
    ...
    let fixed = 6 + ctrl_title.chars().count()
        + if active_label.is_empty() { 0 } else { active_label.chars().count() + 2 };
    ...
    if !active_label.is_empty() {
        spans.push(Span::styled(format!(" {active_label} "), Style::default().fg(Color::DarkGray)));
    }
    spans.push(Span::styled("─┐".to_string(), ctrl_bs));
    out.push(Line::from(spans));
    out.push(Line::from(Span::styled(format!("│{}│", " ".repeat(bw)), ctrl_bs)));
    ...
    // where EEEE is the 4-char readout block.
    {
        ...
        let vert = |x: u16, control: RangeControl, label: &'static str, midi: crate::midi::Action| -> ClickTarget {
            ClickTarget::new(x, y_min, 1, ml as u16,
                ClickAction::SetVerticalRange { control, y_min, y_max })
                .labeled(label)
                .bindable(midi)
        };
        click_targets.push(vert(ta_x, RangeControl::TempoA,  "tempo_a",  crate::midi::Action::Tempo { is_a: true }));
        click_targets.push(vert(va_x, RangeControl::VolumeA, "volume_a", crate::midi::Action::ChannelFader { is_a: true }));
        click_targets.push(vert(vb_x, RangeControl::VolumeB, "volume_b", crate::midi::Action::ChannelFader { is_a: false }));
        click_targets.push(vert(tb_x, RangeControl::TempoB,  "tempo_b",  crate::midi::Action::Tempo { is_a: false }));
        ...
        let cell = |x: u16, row_y: u16, h: u16, section: CtrlSection, label: &'static str, midi: crate::midi::Action| -> ClickTarget {
        ...
        if ml >= 7 {
            ...
            click_targets.push(cell(eq_a_x, y0,     2, EqHighA, "eq_high_a", crate::midi::Action::EqHigh { is_a: true }));
            click_targets.push(cell(eq_a_x, y0 + 2, 2, EqMidA,  "eq_mid_a",  crate::midi::Action::EqMid  { is_a: true }));
            click_targets.push(cell(eq_a_x, y0 + 4, 2, EqLowA,  "eq_low_a",  crate::midi::Action::EqLow  { is_a: true }));
            click_targets.push(cell(eq_a_x, y0 + 6, 1, FilterA, "filter_a", crate::midi::Action::Filter { is_a: true }));
            click_targets.push(cell(eq_b_x, y0,     2, EqHighB, "eq_high_b", crate::midi::Action::EqHigh { is_a: false }));
            click_targets.push(cell(eq_b_x, y0 + 2, 2, EqMidB,  "eq_mid_b",  crate::midi::Action::EqMid  { is_a: false }));
            click_targets.push(cell(eq_b_x, y0 + 4, 2, EqLowB,  "eq_low_b",  crate::midi::Action::EqLow  { is_a: false }));
            click_targets.push(cell(eq_b_x, y0 + 6, 1, FilterB, "filter_b", crate::midi::Action::Filter { is_a: false }));
        }
    }
    ...
    for i in 0..ml {
        ...
        let va = with_underline(fader_ch_vol(i, ml, info.channel_fader_a), CtrlSection::VolumeA);
        let vb = with_underline(fader_ch_vol(i, ml, info.channel_fader_b), CtrlSection::VolumeB);
        let ua = vu_ch(rfb, (la * ml as f64) as usize, ml);
        let ub = vu_ch(rfb, (lb * ml as f64) as usize, ml);
        let da = bdot(i, cr, a_on, has_a, both, aph, info.deck_a_beat_pos, info.downbeat_aligned);
        let db = bdot(i, cr, b_on, has_b, both, aph, info.deck_b_beat_pos, info.downbeat_aligned);
        let cl = if i == cr {
            if both { Span::styled("─", Style::default().fg(pc_aligned(info.phase_offset_ms, info.downbeat_aligned))) }
            else { dim("─") }
        } else { Span::raw(" ") };
        ...
        let eq_cell = |row_idx: usize, is_a: bool| -> String {
            let (l, m, h, f) = if is_a {
                (info.deck_a_eq_low_db, info.deck_a_eq_mid_db,
                 info.deck_a_eq_high_db, info.deck_a_filter_pos)
            } else {
                (info.deck_b_eq_low_db, info.deck_b_eq_mid_db,
                 info.deck_b_eq_high_db, info.deck_b_filter_pos)
            };
            ...
            if is_a { format!("{raw:<4}") } else { format!("{raw:>4}") }
        };
        ...
        let eq_style = |is_a: bool, row_idx: usize| -> Style {
            ...
            let section = match (is_a, row_idx) {
                (true,  0) => Some(EqHighA), (true,  2) => Some(EqMidA),
                (true,  4) => Some(EqLowA),  (true,  6) => Some(FilterA),
                (false, 0) => Some(EqHighB), (false, 2) => Some(EqMidB),
                (false, 4) => Some(EqLowB),  (false, 6) => Some(FilterB),
                _ => None,
            };
        ...
        let mut row: Vec<Span> = vec![
            Span::styled("│ ".to_string(), ctrl_bs),
            ta,
            Span::raw("  "),
        ];
        row.extend(l_spans.iter().cloned());
        if l_pad > 0 { row.push(Span::raw(" ".repeat(l_pad))); }
        row.extend([
            Span::raw("  "),
            Span::styled(eq_a_txt, eq_style(true, i)),
            Span::raw(" "),
            va, Span::raw(" "), ua, Span::raw(" "),
            da, cl, db,
            Span::raw(" "), ub, Span::raw(" "), vb,
            Span::raw(" "),
            Span::styled(eq_b_txt, eq_style(false, i)),
            Span::raw("  "),
        ]);
        row.extend(r_spans.iter().cloned());
        if r_pad > 0 { row.push(Span::raw(" ".repeat(r_pad))); }
        ...
        if end_pad > 0 { row.push(Span::raw(" ".repeat(end_pad))); }
        row.extend([
            Span::raw("  "),
            tb,
            Span::styled(" │".to_string(), ctrl_bs),
    ...
    if has_a && has_b && wf_mode != WaveformMode::Off {
        ...
        let ea = info.deck_a_bpm.map(|b| format!("{:.1}", b)).unwrap_or("—".into());
        let eb = info.deck_b_bpm.map(|b| format!("{:.1}", b)).unwrap_or("—".into());
        ...
        let hdr = format!(" [{}]  A({ea}) → B({eb})  phase: {ph_s}  [w] mode{zoom_label}", wf_mode.label());
        ...
        if let Some(is_a) = waveform_zoom {
            ...
            match wf_mode {
                WaveformMode::Phrase => {
                    ...
                    let phr = if is_a {
                        info.deck_a_analysis.as_ref().map(|a| &a.phrases[..]).unwrap_or(&empty_phr)
                    } else {
                        info.deck_b_analysis.as_ref().map(|a| &a.phrases[..]).unwrap_or(&empty_phr)
                    };
                ...
                WaveformMode::Audio => {
                    ...
                    let wf = if is_a {
                        info.deck_a_analysis.as_ref().map(|a| &a.waveform_peaks[..]).unwrap_or(&empty_wf)
                    } else {
                        info.deck_b_analysis.as_ref().map(|a| &a.waveform_peaks[..]).unwrap_or(&empty_wf)
                    };
            ...
            click_targets.push(
                crate::tui::app::ClickTarget::new(area.x, wf_row_y, area.width, 1,
                    crate::tui::app::ClickAction::WaveformZoom(is_a))
                .labeled(if is_a { "waveform_zoom_a" } else { "waveform_zoom_b" }));
        } else {
            // Normal stacked view
            match wf_mode {
                WaveformMode::Phrase => {
                    let empty_phr: Vec<Phrase> = Vec::new();
                    let a_phr = info.deck_a_analysis.as_ref().map(|a| &a.phrases[..]).unwrap_or(&empty_phr);
                    let b_phr = info.deck_b_analysis.as_ref().map(|a| &a.phrases[..]).unwrap_or(&empty_phr);
                    let sp_a = render_sparkline(a_phr, info.deck_a_time, info.deck_a_duration, wfw, "A");
                    let sp_b = render_sparkline(b_phr, info.deck_b_time, info.deck_b_duration, wfw, "B");
                    out.push(boxed_row(&format!("  {sp_a}"), bw, ctrl_bs));
                    out.push(boxed_row(&format!("  {sp_b}"), bw, ctrl_bs));
                }
                WaveformMode::Audio => {
                    let empty_wf: Vec<f32> = Vec::new();
                    let a_wf = info.deck_a_analysis.as_ref().map(|a| &a.waveform_peaks[..]).unwrap_or(&empty_wf);
                    let b_wf = info.deck_b_analysis.as_ref().map(|a| &a.waveform_peaks[..]).unwrap_or(&empty_wf);
                    let wf_a = render_ascii_waveform(a_wf, info.deck_a_time, info.deck_a_duration, wfw, "A");
                    let wf_b = render_ascii_waveform(b_wf, info.deck_b_time, info.deck_b_duration, wfw, "B");
            ...
            if wf_mode != WaveformMode::Off {
                ...
                click_targets.push(
                    crate::tui::app::ClickTarget::new(area.x, wf_row_y + 1, area.width, 1,
    ...
    for j in 0..mw { xf_meter.push(if j == xf_pos { '║' } else { '─' }); }
    ...
    for j in 0..mw {
        if j == cx { ph_meter.push('│'); }
        else if j == needle && info.phase_offset_ms.abs() > 3.0 { ph_meter.push('▼'); }
        else { ph_meter.push('─'); }
    }
    ...
    let sl = |s: CtrlSection, text: &str| -> String {
        if ss == Some(s) { format!("[{text}]") } else { text.to_string() }
    };
    ...
    if info.state == EngineState::Crossfading {
        ...
        let elapsed_beats = (info.fader_sweep_progress
            * total_beats as f64)
            .floor()
        ...
        out.push(lit_boxed_row(&build_ctrl_row("", &label, "", content_w), bw, ctrl_bs));
    ...
    // span maps linearly to crossfader [-1, +1].
    {
        ...
        click_targets.push(
            crate::tui::app::ClickTarget::new(x_min, area.y + xf_row, mw as u16, 1,
    ...
    let push_first = |targets: &mut Vec<ClickTarget>, row_str: &str, label: &str, y: u16,
                      key: KeyCode, tag: &'static str| -> Option<usize> {
        let byte_pos = row_str.find(label)?;
        let char_col: usize = row_str[..byte_pos].chars().count();
        let label_w = label.chars().count() as u16;
        targets.push(
            ClickTarget::new(row_x_base + char_col as u16, y, label_w.max(1), 1,
                ClickAction::SimulateKey(key))
            .labeled(tag));
        Some(byte_pos + label.len())
    };
    let push_second = |targets: &mut Vec<ClickTarget>, row_str: &str, label: &str, y: u16,
                       key: KeyCode, tag: &'static str, after: usize| {
    ...
    if let Some(after) = push_first(click_targets, &row1, "CUE", row1_y,
        KeyCode::Null, "cue_a")
    {
        ...
        if let Some(last) = click_targets.last_mut() {
            ...
            last.midi_action = Some(crate::midi::Action::CueSet { is_a: true, slot: 0 });
        }
        push_second(click_targets, &row1, "CUE", row1_y,
            KeyCode::Null, "cue_b", after);
        if let Some(last) = click_targets.last_mut()
            && last.label == Some("cue_b") {
                last.action = ClickAction::FocusDashSection(CtrlSection::CueB);
                last.midi_action = Some(crate::midi::Action::CueSet { is_a: false, slot: 0 });
            }
    }
    ...
    // then derive sub-rects from char positions.
    {
        ...
        let push_jump_targets = |targets: &mut Vec<ClickTarget>, byte_pos: usize, is_a: bool| {
            ...
            targets.push(
                ClickTarget::new(x_left, row1_y, 1, 1,
    ...
    if let Some(after) = push_first(click_targets, &row2, "PLAY", row2_y,
        KeyCode::Char('p'), "play_a")
    {
        if let Some(last) = click_targets.last_mut() {
            last.midi_action = Some(crate::midi::Action::PlayPauseDeck { is_a: true });
        }
        push_second(click_targets, &row2, "PLAY", row2_y,
            KeyCode::Char('p'), "play_b", after);
        if let Some(last) = click_targets.last_mut()
    ...
    // so the user can map ◀ and ▶ separately.
    {
        ...
        let push_nudge_targets = |targets: &mut Vec<ClickTarget>, byte_pos: usize, is_a: bool| {
            ...
            targets.push(
                ClickTarget::new(x_left, row2_y, 1, 1,
                    ClickAction::SimulateKey(KeyCode::Char('[')))
            ...
            let _ = (x_mid, mid_w);
            targets.push(
                ClickTarget::new(x_right, row2_y, 1, 1,
                    ClickAction::SimulateKey(KeyCode::Char(']')))
        ...
        for is_a in [true, false] {
            ...
            if let Some((bp, len)) = byte_pos {
                ...
                let tag: &'static str = match (is_a, slot) {
                    (true,  1) => "hot_cue_1_a", (true,  2) => "hot_cue_2_a",
                    (true,  3) => "hot_cue_3_a", (true,  4) => "hot_cue_4_a",
                    (false, 1) => "hot_cue_1_b", (false, 2) => "hot_cue_2_b",
                    (false, 3) => "hot_cue_3_b", (false, 4) => "hot_cue_4_b",
                    _ => "hot_cue_unknown",
                };
                click_targets.push(
                    ClickTarget::new(row_x_base + col, row1_y, 3, 1,
    ...
    // chars: 4 + 3 + "◀ NUDGE ▶"(9) + 3 = 19 prefix; loop strip 19.
    {
        ...
        let buttons: [(f64, u16, u16); 5] = [
            (1.0, 0, 1), (2.0, 3, 1), (4.0, 6, 1), (8.0, 9, 1), (16.0, 12, 2),
        ];
        let off_off: u16 = 16; let off_w: u16 = 3;
        ...
        for (beats, off_in_strip, w) in buttons {
            click_targets.push(crate::tui::app::ClickTarget::new(
                a_strip_x + off_in_strip, row2_y, w, 1,
                crate::tui::app::ClickAction::LoopEngageDeck { is_a: true, beats },
            ).labeled(match beats as u32 {
                1 => "loop_1_a", 2 => "loop_2_a", 4 => "loop_4_a",
                8 => "loop_8_a", 16 => "loop_16_a", _ => "loop_n_a",
            }));
        }
        click_targets.push(crate::tui::app::ClickTarget::new(
            a_strip_x + off_off, row2_y, off_w, 1,
        ...
        for (beats, off_in_strip, w) in buttons {
            click_targets.push(crate::tui::app::ClickTarget::new(
                b_strip_x + off_in_strip, row2_y, w, 1,
                crate::tui::app::ClickAction::LoopEngageDeck { is_a: false, beats },
            ).labeled(match beats as u32 {
                1 => "loop_1_b", 2 => "loop_2_b", 4 => "loop_4_b",
                8 => "loop_8_b", 16 => "loop_16_b", _ => "loop_n_b",
            }));
        }
        click_targets.push(crate::tui::app::ClickTarget::new(
            b_strip_x + off_off, row2_y, off_w, 1,
    ...
    out.push(lit_boxed_row(&format!("{}{pv}", " ".repeat(phase_pad)), bw, ctrl_bs));
    ...
    match dash_layout {
        DashLayout::Full => {
            ...
            render_queue_section(&mut out, info, download_in_flight, w, cw, dash_focus == DashFocus::Queue);
            render_history_section(&mut out, info, w, cw, dash_focus == DashFocus::History);
            render_browse_section(&mut out, area, browse_items, browse_breadcrumb, browse_selected, browse_is_tracks, w, click_targets, dash_focus == DashFocus::Browse);
            render_log_section(&mut out, log_scroll_offset, w, h, dash_focus == DashFocus::Log);
        }
        DashLayout::Panel => {
...
fn deck_lines_styled<'a>(info: &NowPlayingInfo, is_a: bool, dw: usize, bw: usize) -> Vec<(Vec<Span<'a>>, usize)> {
    // is_a now means "physical deck A" — the always-left deck.
    let track = if is_a { &info.deck_a_track } else { &info.deck_b_track };
    let bpm = if is_a { info.deck_a_bpm } else { info.deck_b_bpm };
    let time = if is_a { info.deck_a_time } else { info.deck_b_time };
    let dur = if is_a { info.deck_a_duration } else { info.deck_b_duration };
    let is_playing = if is_a { info.deck_a_is_playing } else { info.deck_b_is_playing };
    let kick = if is_a { info.deck_a_kick } else { info.deck_b_kick };
    ...
    match track {
        Some(t) => {
            ...
            let pct = bpm.map(|b| format!("({:+.1}%)", (b/nat-1.0)*100.0)).unwrap_or_default();
            ...
            // Line 3: 132 BPM / 6A
            let info_str = format!("{} BPM / {}", t.bpm.map(|b| format!("{:.0}", b)).unwrap_or("?".into()), t.key.as_deref().unwrap_or("?"));
            ...
            let empty_wf: Vec<f32> = Vec::new();
            let empty_phr: Vec<Phrase> = Vec::new();
            let analysis = if is_a { &info.deck_a_analysis } else { &info.deck_b_analysis };
            let peaks = analysis.as_ref().map(|a| &a.waveform_peaks[..]).unwrap_or(&empty_wf);
            lines.push(render_waveform_spans(peaks, time, dur, bw));

            // Line 7: Sparkline (colored, with mix point on the playing deck only)
            let phrases = analysis.as_ref().map(|a| &a.phrases[..]).unwrap_or(&empty_phr);
            let mix_pt = if is_playing { info.mix_point_time } else { None };
            lines.push(render_sparkline_spans(phrases, time, dur, bw, mix_pt));
        }
        None => {
            // Empty deck
            let l1 = vec![Span::styled(format!("{lbl} "), bold), Span::styled(live_ch, dk)];
...
fn render_waveform_spans<'a>(peaks: &[f32], cur: f64, total: f64, w: usize) -> (Vec<Span<'a>>, usize) {
    let wf = [' ', '▁', '▂', '▃', '▄', '▅', '▆', '▇', '█'];
    let dk = Style::default().fg(Color::DarkGray);
    let white = Style::default().fg(Color::White);
    let cyan = Style::default().fg(Color::Cyan);

    if peaks.is_empty() { return (vec![Span::styled("░".repeat(w), dk)], w); }
    let cursor = if total > 0.0 { (cur / total * w as f64) as usize } else { 0 };
    ...
    for col in 0..w {
        ...
        let style = if col == cursor { cyan } else if col < cursor { dk } else { white };
        let ch = wf[ci];
    ...
    if !cur_text.is_empty() { spans.push(Span::styled(cur_text, cur_style)); }
    (spans, w)
}

fn render_sparkline_spans<'a>(phrases: &[Phrase], cur: f64, total: f64, w: usize, mix_point: Option<f64>) -> (Vec<Span<'a>>, usize) {
    let sp = ['▁', '▂', '▃', '▄', '▅', '▆', '▇', '█'];
    let dk = Style::default().fg(Color::DarkGray);
    let white = Style::default().fg(Color::White);
    let green = Style::default().fg(Color::Green);
    ...
    let cursor = if total > 0.0 { (cur / total * w as f64) as usize } else { 0 };
    let mix_col = mix_point.and_then(|mp| if total > 0.0 { Some((mp / total * w as f64) as usize) } else { None });
    let mut spans: Vec<Span> = Vec::new();
    let mut cur_style = dk;
    let mut cur_text = String::new();
    ...
    for col in 0..w {
        ...
        let pi = phrases.partition_point(|p| p.start_time <= t).saturating_sub(1);
        let energy = if pi < phrases.len() { phrases[pi].energy } else { 0.0 };
        let ci = (energy * sp.len() as f64).min(sp.len() as f64 - 1.0).max(0.0) as usize;
...
fn render_sparkline(phrases: &[Phrase], cur: f64, total: f64, w: usize, label: &str) -> String {
    let chars = ['▁', '▂', '▃', '▄', '▅', '▆', '▇', '█'];
    if phrases.is_empty() { return format!("{label}: {}", "░".repeat(w)); }
    let cursor = if total > 0.0 { ((cur / total) * w as f64) as usize } else { usize::MAX };
    ...
    for col in 0..w {
        ...
        let pi = phrases.partition_point(|p| p.start_time <= t).saturating_sub(1);
        let energy = if pi < phrases.len() { phrases[pi].energy } else { 0.0 };
        let ci = (energy * chars.len() as f64).min(chars.len() as f64 - 1.0).max(0.0) as usize;
        s.push(chars[ci]);
    }
    s
}
...
fn render_ascii_waveform(peaks: &[f32], cur: f64, total: f64, w: usize, label: &str) -> String {
    let chars = ['_', '.', '-', '~', '^', '/', '\\', '|'];
    if peaks.is_empty() { return format!("{label}: {}", "_".repeat(w)); }
...
fn fader_ch(row: usize, ml: usize, rate: f64) -> Span<'static> {
    ...
    if row == pos { Span::styled("═", Style::default().fg(Color::White)) }
    else { dim("│") }
}
...
fn vu_ch(rfb: usize, thresh: usize, ml: usize) -> Span<'static> {
    if rfb < thresh {
        let top = ml.saturating_sub(1); let yel = ml.saturating_sub(2);
        if rfb == top && thresh > top { Span::styled("█", Style::default().fg(Color::Red)) }
        else if rfb == yel && thresh > yel { Span::styled("█", Style::default().fg(Color::Yellow)) }
...
fn build_ctrl_row(left: &str, center: &str, right: &str, total_w: usize) -> String {
    ...
    format!("{left}{}{center}{}{right}", " ".repeat(gap_l), " ".repeat(gap_r))
}
...
fn boxed_row(content: &str, bw: usize, border: Style) -> Line<'static> {
    ...
    Line::from(vec![
        ...
        Span::styled(ps(content, cw).to_string(), Style::default().fg(Color::DarkGray)),
        Span::styled(" │".to_string(), border),
    ])
}
...
pub fn render_dashboard_text(info: &NowPlayingInfo, wf_mode: WaveformMode, width: usize) -> Vec<String> {
    ...
    for j in 0..mw { xf_bar.push(if j == xf_pos { '║' } else { '─' }); }
    xf_bar.push_str(" B");
    out.push(row(&build_ctrl_row("CUE  ◀ JUMP ▶", &xf_bar, "◀ JUMP ▶  CUE", cw)));
    ...
    for j in 0..mw {
        if j == cx { ph_bar.push('│'); }
        else if j == needle && info.phase_offset_ms.abs() > 3.0 { ph_bar.push('▼'); }
        else { ph_bar.push('─'); }
    }
    let pv2 = format!("{:+.1}ms", info.phase_offset_ms);
    ph_bar.push_str(&format!("  {:<7}", pv2));
    out.push(row(&build_ctrl_row("PLAY  ◀ NUDGE ▶", &ph_bar, "◀ NUDGE ▶  PLAY", cw)));
    ...
    if has_a && has_b && wf_mode != WaveformMode::Off {
        ...
        match wf_mode {
            WaveformMode::Audio => {
                out.push(row(&render_ascii_waveform(a_wf, info.deck_a_time, info.deck_a_duration, wfw, "A")));
                out.push(row(&render_ascii_waveform(b_wf, info.deck_b_time, info.deck_b_duration, wfw, "B")));
            }
            WaveformMode::Phrase => {
                out.push(row(&render_sparkline(a_phr, info.deck_a_time, info.deck_a_duration, wfw, "A")));
                out.push(row(&render_sparkline(b_phr, info.deck_b_time, info.deck_b_duration, wfw, "B")));
            }
            WaveformMode::Off => {}
        }
    }
    ...
    if info.queue.is_empty() {
        ...
        for (i, e) in info.queue.iter().take(10).enumerate() {
            let bk = format!("{}/{}",
                e.track.bpm.map(|b| format!("{:.0}", b)).unwrap_or_else(|| "?".into()),
                e.track.key.as_deref().unwrap_or("?"));
            out.push(row(&format!("{:>2}  {} - {}  {bk}", i + 1, e.track.artist_name(), e.track.full_title())));
        }
    ...
    if info.history.is_empty() {
        ...
        for (i, e) in info.history.iter().take(10).enumerate() {
            let bk = format!("{}/{}",
                ...
                e.track.key.as_deref().unwrap_or("?"));
            let mix = e.mix_score.map(|s| format!(" m{s}")).unwrap_or_default();
            out.push(row(&format!("{:>2}  {} - {}  {bk}{mix}", i + 1, e.track.artist_name(), e.track.full_title())));
        }
    ...
    // === State ===
    out.push(format!("State: {:?}  Phase: {:+.1}ms  Crossfade: {:.0}%", info.state, info.phase_offset_ms, info.crossfade_progress * 100.0));
    out.push(format!("Playing:  {} BPM  {}", info.playing_bpm.map(|b| format!("{:.1}", b)).unwrap_or("—".into()), info.playing_track.as_ref().map(|t| format!("{} - {}", t.artist_name(), t.full_title())).unwrap_or("—".into())));
    out.push(format!("Incoming: {} BPM  {}", info.incoming_bpm.map(|b| format!("{:.1}", b)).unwrap_or("—".into()), info.incoming_track.as_ref().map(|t| format!("{} - {}", t.artist_name(), t.full_title())).unwrap_or("—".into())));
...
fn deck_text_line(info: &NowPlayingInfo, is_a: bool, dw: usize) -> Vec<String> {
    ...
    match track {
        Some(t) => vec![
            format!("{label} {} BPM", bpm.map(|b| format!("{:.1}", b)).unwrap_or("—".into())),
...
fn read_logs_offset(count: usize, offset_back: usize) -> Vec<String> {
    ...
    struct Cache { last_len: u64, lines: Vec<String>, last_stat: Option<Instant> }
    static CACHE: Mutex<Cache> = Mutex::new(Cache { last_len: 0, lines: Vec::new(), last_stat: None });
    ...
    if len != cache.last_len {
        ...
        cache.lines = buf.lines()
            .filter(|l| !l.is_empty())
            .map(|l| if let Some(b) = l.find(']') { l[b+1..].trim().to_string() } else { l.to_string() })
            .collect();
        cache.last_len = len;
    }
        ...
        for w in [40usize, 41, 58, 60, 61, 80, 119, 200] {
            ...
            for n in [0usize, 1, 3, 4, 7] {
                let items: Vec<(Color, String)> = (0..n)
                    .map(|i| (Color::Green, format!("track {i} — a fairly long label here")))
                    .collect();
                let overflow = (n > 0).then_some("… 3 more");
                let mut out: Vec<Line> = Vec::new();
                render_two_col_box(&mut out, "QUEUE (9) [q]", &items, overflow, "Empty", w, false);
                assert!(!out.is_empty(), "w={w} n={n}: nothing rendered");
                for (row, line) in out.iter().enumerate() {
                    assert_eq!(
                        line.width(), w,
                        "w={w} n={n}: row {row} is {} cells, expected {w}", line.width(),
                    );
                }
            }
        }
