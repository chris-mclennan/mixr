...
use crate::config::{AnalyzerEngine, AudioQuality};
    ...
    tokio::spawn(async move {
        let (bytes, ext) = if let Some(path) = track.local_path.clone() {
            ...
            let bytes = match bytes_result {
                Ok(Ok(b)) => b,
                Ok(Err(e)) => {
                    tx.send(AppAction::Toast(format!("Local read: {e}"))).ok();
                    return;
                }
                Err(_) => return,
            };
            let ext: &'static str = match track
                .local_path
                .as_deref()
            ...
            let source = match api_guard.get_track_source(track.id, quality).await {
                Ok(s) => s,
                Err(e) => {
                    tx.send(AppAction::Toast(format!("Source error: {e}"))).ok();
                    return;
                }
            };
            drop(api_guard);
            let dl = match downloader.download(&source, track.id).await {
                Ok(d) => d,
                Err(e) => {
                    tx.send(AppAction::Toast(format!("Download failed: {e}")))
                        .ok();
                    return;
                }
            };
        ...
        .await
        {
            Ok(Ok((analysis, samples, sample_rate))) => {
                tx.send(AppAction::PreviewReady {
                    samples,
                    sample_rate,
                    analysis,
                })
            ...
            Ok(Err(e)) => {
                tx.send(AppAction::Toast(format!("Analysis failed: {e}")))
                    .ok();
            }
            Err(e) => {
                tx.send(AppAction::Toast(format!("Task failed: {e}"))).ok();
            }
        }
    });
}
...
pub fn download_and_play(p: Pipeline, track: BeatportTrack, as_incoming: bool) {
    ...
    tokio::spawn(async move {
        ...
        let (bytes, ext, source_label) = if let Some(path) = track.local_path.clone() {
            tx.send(AppAction::Toast(format!("📂 {short_name} (LOCAL)")))
                .ok();
            let bytes_result = tokio::task::spawn_blocking(move || std::fs::read(&path)).await;
            let bytes = match bytes_result {
                Ok(Ok(b)) => b,
                Ok(Err(e)) => {
                    tx.send(AppAction::DownloadFailed(format!("Local file read: {e}")))
                        .ok();
                    return;
                }
                Err(e) => {
                    tx.send(AppAction::DownloadFailed(format!("Task panicked: {e}")))
                        .ok();
                    return;
                }
            };
            ...
            tx.send(AppAction::Toast(format!("⬇ {short_name} ({label})")))
                .ok();
            let dl = match downloader.download(&source, track.id).await {
                Ok(d) => d,
                Err(e) => {
                    tx.send(AppAction::DownloadFailed(format!("Download: {e}")))
                        .ok();
                    return;
                }
            };
        ...
        tx.send(AppAction::Toast(format!(
            "Analyzing {short_name} ({size_kb}KB in {dl_secs:.1}s)"
        )))
        .ok();
        ...
        match result {
            Ok(Ok((mut analysis, samples, sample_rate))) => {
                ...
                if any_ai {
                    tx.send(AppAction::Toast(format!("AI analysis: {short_name}")))
                        .ok();
                }

                // 1. AI beat onset refinement
                if ai_beat {
                    let (refined_beat, reason) = crate::audio::ai_beat::detect_combined(
                        &samples,
                        sample_rate,
                    ...
                    if diff > 1.0 {
                        ...
                        tracing::info!(
                            "AI refined first beat: {old:.3}s → {refined_beat:.3}s ({diff:.1}ms, {reason})"
                        );
                    } else {
                    ...
                    .await
                    {
                        Ok(detected_phrases) if !detected_phrases.is_empty() => {
                            ...
                            if detected_phrases.len() >= dsp_count {
                                analysis.phrases = detected_phrases
                                    .iter()
                                    .map(|(time, label)| {
                ...
                tx.send(AppAction::TrackDecoded {
                    track,
                    samples,
                    sample_rate,
                    analysis,
            ...
            Ok(Err(e)) => {
                tx.send(AppAction::DownloadFailed(format!("Analysis: {e}")))
                    .ok();
            }
            Err(e) => {
                tx.send(AppAction::DownloadFailed(format!("Task: {e}")))
                    .ok();
            }
        }
    });
