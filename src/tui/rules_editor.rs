...
pub const TRANSITIONS: &[&str] = &["BeatMatched", "EchoOut", "BassSwap", "FilterSweep", "LoopRoll"];

/// Condition fields the UI exposes. `None` means "no condition" (matches always).
pub const CONDITION_FIELDS: &[&str] = &[
    "(any — matches always)",
    "bpm_gap_pct_gt", "bpm_gap_pct_lt",
    "key_dist_eq", "key_dist_lte", "key_dist_gte",
    "last_transition_eq",
    "mix_count_mod",
    "energy_delta_gt", "energy_delta_lt",
    "phrase_is_drop",
    "time_in_set_min_gt", "time_in_set_min_lt",
];

pub const ACTION_KINDS: &[&str] = &["force", "cycle", "weighted"];

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Focus { When, Then, List }
...
impl RulesEditor {
    pub fn new(config: RuleConfig) -> Self {
        Self { config, selected: 0, mode: Mode::List, dirty: false }
    }
}
...
fn blank_condition() -> Condition { Condition::default() }
fn blank_rule() -> Rule { Rule { when: blank_condition(), then: Action::Force("BeatMatched".into()) } }
...
pub fn condition_field_idx(c: &Condition) -> usize {
    if c.bpm_gap_pct_gt.is_some() { return 1; }
    if c.bpm_gap_pct_lt.is_some() { return 2; }
    if c.key_dist_eq.is_some() { return 3; }
    if c.key_dist_lte.is_some() { return 4; }
    if c.key_dist_gte.is_some() { return 5; }
    if c.last_transition_eq.is_some() { return 6; }
    if c.mix_count_mod.is_some() { return 7; }
    if c.energy_delta_gt.is_some() { return 8; }
    if c.energy_delta_lt.is_some() { return 9; }
    if c.phrase_is_drop.is_some() { return 10; }
    if c.time_in_set_min_gt.is_some() { return 11; }
    if c.time_in_set_min_lt.is_some() { return 12; }
    0
}
...
pub fn condition_value_str(c: &Condition, field_idx: usize) -> String {
    match field_idx {
        ...
        7 => c.mix_count_mod.map(|[m, r]| format!("{m},{r}")).unwrap_or_default(),
        8 => c.energy_delta_gt.map(|v| format!("{v}")).unwrap_or_default(),
        9 => c.energy_delta_lt.map(|v| format!("{v}")).unwrap_or_default(),
        10 => c.phrase_is_drop.map(|v| v.to_string()).unwrap_or_default(),
        11 => c.time_in_set_min_gt.map(|v| format!("{v}")).unwrap_or_default(),
        12 => c.time_in_set_min_lt.map(|v| format!("{v}")).unwrap_or_default(),
        _ => String::new(),
    }
}
...
pub fn adjust_condition_value(c: &mut Condition, field_idx: usize, step: i32) {
    match field_idx {
        1 => { if let Some(ref mut v) = c.bpm_gap_pct_gt { *v = (*v + step as f64).max(0.0); } }
        2 => { if let Some(ref mut v) = c.bpm_gap_pct_lt { *v = (*v + step as f64).max(0.0); } }
        3 => { if let Some(ref mut v) = c.key_dist_eq { let n = *v as i32 + step; *v = n.clamp(0, 6) as usize; } }
        4 => { if let Some(ref mut v) = c.key_dist_lte { let n = *v as i32 + step; *v = n.clamp(0, 6) as usize; } }
        5 => { if let Some(ref mut v) = c.key_dist_gte { let n = *v as i32 + step; *v = n.clamp(0, 6) as usize; } }
        6 => {
        ...
        8 => { if let Some(ref mut v) = c.energy_delta_gt { *v = (*v + 0.05 * step as f64).clamp(-1.0, 1.0); } }
        9 => { if let Some(ref mut v) = c.energy_delta_lt { *v = (*v + 0.05 * step as f64).clamp(-1.0, 1.0); } }
        10 => { if let Some(ref mut v) = c.phrase_is_drop { *v = !*v; let _ = step; } }
        11 => { if let Some(ref mut v) = c.time_in_set_min_gt { let n = *v as i32 + step * 5; *v = n.max(0) as u32; } }
        12 => { if let Some(ref mut v) = c.time_in_set_min_lt { let n = *v as i32 + step * 5; *v = n.max(0) as u32; } }
        _ => {}
    }
}
...
pub fn build_action(kind_idx: usize, choices: &[(String, f64)]) -> Action {
    ...
    match kind_idx {
        ...
        1 => Action::Cycle { cycle: choices.iter().map(|(s, _)| s.clone()).collect() },
...
pub fn describe_condition(c: &Condition) -> String {
    ...
    if idx == 0 { return "(any)".into(); }
    let v = condition_value_str(c, idx);
    format!("{} {}", CONDITION_FIELDS[idx], if v.is_empty() { "—" } else { v.as_str() })
}
...
pub fn describe_action(a: &Action) -> String {
    match a {
        ...
        Action::Weighted { weighted } => {
            ...
            let parts: Vec<String> = weighted.iter()
...
fn render_list(frame: &mut Frame, area: Rect, editor: &RulesEditor) {
    ...
    let def_line = format!("{marker}Default                                                  →  {}", editor.config.default);
...
fn render_edit(frame: &mut Frame, area: Rect, state: &EditState) {
    let mut lines: Vec<Line> = Vec::new();
    let title = match state.idx { Some(i) => format!("Edit Rule {}", i + 1), None => "New Rule".into() };
    lines.push(Line::from(Span::styled(
        title,
    ...
    let highlight = |txt: String, on: bool| {
        if on { Span::styled(txt, Style::default().fg(Color::Black).bg(Color::Yellow).add_modifier(Modifier::BOLD)) }
        else  { Span::styled(txt, Style::default().fg(Color::Cyan)) }
    };
    lines.push(Line::from(Span::styled(
        pad_head(if when_focused { "▸ When:" } else { "  When:" }),
        Style::default().fg(if when_focused { Color::Yellow } else { Color::Gray }).add_modifier(Modifier::BOLD),
    )));
    ...
    lines.push(Line::from(vec![
        Span::raw("      Value:   "),
        Span::styled(if field_val.is_empty() { "—".into() } else { format!("[ {field_val} ]") },
            Style::default().fg(if when_focused { Color::Yellow } else { Color::Gray })),
    ]));
    lines.push(Line::from(""));
    ...
    lines.push(Line::from(Span::styled(
        pad_head(if then_focused || list_focused { "▸ Then:" } else { "  Then:" }),
        Style::default().fg(if then_focused || list_focused { Color::Yellow } else { Color::Gray }).add_modifier(Modifier::BOLD),
    )));
    lines.push(Line::from(vec![
        Span::raw("      Kind:   "),
    ...
    let total: f64 = choices.iter().map(|(_, w)| w.max(0.0)).sum::<f64>().max(1e-9);
...
fn volume_curves(name: &str) -> Option<(String, String)> {
    ...
    for i in 0..N {
        ...
        let idx_play = ((v_play.clamp(0.0, 1.0) * (GLYPHS.len() - 1) as f64) as usize).min(GLYPHS.len() - 1);
        let idx_in = ((v_in.clamp(0.0, 1.0) * (GLYPHS.len() - 1) as f64) as usize).min(GLYPHS.len() - 1);
    ...
    match editor.mode.clone() {
        Mode::List => match key.code {
            ...
            KeyCode::Down => {
                ...
                if editor.selected < max { editor.selected += 1; }
                KeyResult::Continue
            }
            KeyCode::Enter => {
                if editor.selected < editor.config.rules.len() {
                    ...
                    editor.mode = Mode::Edit(EditState {
                        idx: Some(editor.selected), rule, focus: Focus::When, action_selected: 0,
                    });
                } else {
                    // Editing the Default row = cycle transition.
                    let cur = TRANSITIONS.iter().position(|t| t == &editor.config.default.as_str()).unwrap_or(0);
            ...
            KeyCode::Char('i') => {
                editor.mode = Mode::Edit(EditState {
                    idx: None, rule: blank_rule(), focus: Focus::When, action_selected: 0,
                });
                KeyResult::Continue
            }
            KeyCode::Char('D') => {
            ...
            KeyCode::Char('{') => {
                if editor.selected > 0 && editor.selected < editor.config.rules.len() {
                    editor.config.rules.swap(editor.selected, editor.selected - 1);
                    editor.selected -= 1;
                    editor.dirty = true;
                }
                KeyResult::Continue
        ...
        Mode::Edit(mut state) => {
            let result = match key.code {
                ...
                KeyCode::Enter => {
                    match state.idx {
                        Some(i) => { editor.config.rules[i] = state.rule.clone(); }
                        None => { editor.config.rules.push(state.rule.clone()); editor.selected = editor.config.rules.len() - 1; }
                    }
                    editor.dirty = true;
                    editor.mode = Mode::List;
                ...
                KeyCode::Left | KeyCode::Right => {
                    let step = if matches!(key.code, KeyCode::Right) { 1 } else { -1 };
                    match state.focus {
                        Focus::When => {
                            let cur = condition_field_idx(&state.rule.when);
                            let next = (cur as i32 + step).rem_euclid(CONDITION_FIELDS.len() as i32) as usize;
                            set_condition_field(&mut state.rule.when, next);
                        }
                        Focus::Then => {
                            let cur = action_kind_idx(&state.rule.then);
                            let next = (cur as i32 + step).rem_euclid(ACTION_KINDS.len() as i32) as usize;
                            let choices = action_choices(&state.rule.then);
                        ...
                        Focus::List => {
                            match state.rule.then.clone() {
                                ...
                                Action::Cycle { mut cycle } if !cycle.is_empty() => {
                                    ...
                                    let cur = TRANSITIONS.iter().position(|t| t == &cycle[i].as_str()).unwrap_or(0);
                                    let next = (cur as i32 + step).rem_euclid(TRANSITIONS.len() as i32) as usize;
                                    cycle[i] = TRANSITIONS[next].into();
                                    state.rule.then = Action::Cycle { cycle };
                                }
                                Action::Force(cur) => {
                                    let i = TRANSITIONS.iter().position(|t| t == &cur.as_str()).unwrap_or(0);
                                    let next = (i as i32 + step).rem_euclid(TRANSITIONS.len() as i32) as usize;
                ...
                KeyCode::Up | KeyCode::Down => {
                    let step: i32 = if matches!(key.code, KeyCode::Up) { 1 } else { -1 };
                    match state.focus {
                        Focus::When => {
                            let field_idx = condition_field_idx(&state.rule.when);
                        ...
                        Focus::List => {
                            ...
                            if len > 0 {
                                let next = (state.action_selected as i32 - step).rem_euclid(len as i32) as usize;
                ...
                KeyCode::Char('D') => {
                    match state.rule.then.clone() {
                        Action::Weighted { mut weighted } if weighted.len() > 1 => {
                            let i = state.action_selected.min(weighted.len() - 1);
                            weighted.remove(i);
                            if state.action_selected >= weighted.len() { state.action_selected = weighted.len() - 1; }
                            state.rule.then = Action::Weighted { weighted };
                        }
                        Action::Cycle { mut cycle } if cycle.len() > 1 => {
                            let i = state.action_selected.min(cycle.len() - 1);
                            cycle.remove(i);
                            if state.action_selected >= cycle.len() { state.action_selected = cycle.len() - 1; }
                            state.rule.then = Action::Cycle { cycle };
                        }
                        _ => {}
                    }
