...
impl App {
    pub(crate) fn execute_dj_tool(&mut self, tool: &crate::claude::api::ToolCall) -> String {
        match tool.name.as_str() {
            "browse_screen" => {
                ...
                let items: Vec<String> = (0..count.min(20))
                    .map(|i| format!("{}: {}", i, self.current_screen().item_label(i)))
                    .collect();
                    ...
                    let compat = {
                        ...
                        if playing_bpm > 0.0 && track_bpm > 0.0 {
                            ...
                            if gap_pct > 8.0 {
                                ...
                                notes.push(format!(
                                    "BPM {playing_bpm:.0}→{track_bpm:.0} ({gap_pct:.1}%)"
                                ));
                            }
                        }
                        if let (Some(pk), Some(tk)) = (playing_key, track_key) {
                            ...
                            match dist {
                                ...
                                2..=3 => {
                                    notes.push(format!("key {pk}→{tk} (dist {dist} — workable)"))
                                }
                                _ => notes
                                    .push(format!("⚠ key {pk}→{tk} (dist {dist} — risky clash)")),
                            }
                        }
                        if notes.is_empty() {
                            String::new()
            ...
            "nudge" => {
                let dir = if tool.string("direction") == Some("forward") {
                    1
                } else {
                    -1
                };
            ...
            "sweep_crossfader" => {
                ...
                format!(
                    "Sweeping crossfader → {target:+.2} over {bars} bars (engine paces — no further calls needed)"
                )
            }
            "set_channel_fader" => {
            ...
            "seek_deck" => {
                ...
                if let Some(secs) = tool.float("target") {
                    ...
                    match self.engine.seek_deck_named(is_a, label) {
                        Some(t) => format!(
                            "Deck {} seek to {label} ({t:.2}s)",
                            if is_a { "A" } else { "B" }
                        ),
                        None => format!(
                            "Deck {} seek to '{label}' failed — unknown label or no analysis yet",
                            if is_a { "A" } else { "B" }
                        ),
                    }
                } else {
            ...
            "read_alignment" => {
                ...
                if a.beat_phase_ms.abs() > 5.0 {
                    notes.push(format!(
                        "phase {:+.1}ms — adjust tempo or nudge",
                        a.beat_phase_ms
                    ));
                }
                if one_mismatch {
                    ...
                    notes.push(format!(
                        "1s are OFF by {} beat(s) — jump_beats to fix",
                        delta
                    ));
                }
                if phrase_mismatch {
                    let delta =
                        (a.bar_in_phrase_b as i32 - a.bar_in_phrase_a as i32).rem_euclid(16);
                    notes.push(format!(
                        "phrase is off by {delta} bar(s) — drops won't land together"
                    ));
                }
                if notes.is_empty() {
                    notes.push("aligned — phase, 1s, and phrase all line up".into());
                }
                format!(
            ...
            "preview_deck" => {
                ...
                if self.config.monitor_device.is_empty() {
                    "preview_deck needs a monitor_device configured — set one via \
                         settings or {\"monitor_device\":\"name\"} IPC"
                        .to_string()
                    ...
                    format!(
                        "Deck {} has no track loaded — nothing to preview",
                        if is_a { "A" } else { "B" }
                    )
                }
            }
            ...
            "play_deck" => {
                ...
                if self.engine.play_deck(is_a) {
                    format!(
                        "Deck {} → live on main output",
                        if is_a { "A" } else { "B" }
                    )
                } else {
                    format!(
                        "Deck {} has no track loaded — load_to_deck first",
                        if is_a { "A" } else { "B" }
                    )
                }
            }
            ...
            "cue" => {
                ...
                match action {
                    ...
                    "clear" => {
                        ...
                        format!("Cue {} cleared ({})", slot + 1, Self::deck_label(is_a))
                    }
                    _ => {
    ...
    pub(crate) fn toggle_claude_dj(&mut self) {
        if let Some(ref dj_arc) = self.claude_dj {
            ...
            self.engine
                .apply_claude_dj_settings(&self.config.claude_dj, self.config.claude_dj_enabled);
            self.config.save();
            if now_on {
            ...
            self.toast
                .show("No Claude API key (~/.mixr/claude_key)", 2.0);
        }
    }
        ...
        tokio::spawn(async move {
            ...
            match dj.trigger(&context, &reason).await {
                Ok(tools) if !tools.is_empty() => {
                    tx.send(AppAction::DjToolCalls(tools)).ok();
                }
                Ok(_) => {} // No tools = Claude is done
                Err(e) => {
                    tx.send(AppAction::Toast(format!("DJ error: {e}"))).ok();
                }
            }
        });
    }
    ...
    pub(crate) fn build_performance_context(&self) -> String {
        ...
        format!(
            ...
            align.beat_phase_ms,
            align.beat_phase_fraction,
            align.beat_in_bar_a,
            align.beat_in_bar_b,
            align.beat_in_bar_a == align.beat_in_bar_b,
            align.bar_in_phrase_a,
            align.bar_in_phrase_b,
    ...
    pub(crate) fn build_dj_context(&self) -> String {
        ...
        let incoming = info
            ...
            .unwrap_or("None".into());
        let remaining = info.playing_duration - info.playing_time;
        let queue_tracks: Vec<String> = info
            .queue
            .iter()
            .take(5)
            .map(|e| {
        ...
        let recent_scores: Vec<String> = info
            .history
            .iter()
            .rev()
            .take(5)
        ...
        let phrase_now = if let Some(p) = info.playing_analysis.as_ref().and_then(|a| {
            a.phrases
                .iter()
                .rev()
                .find(|ph| ph.start_time <= info.playing_time)
        }) {
            format!(
                "{:?} (started {:.0}s ago, energy {:.2})",
                p.phrase_type,
                info.playing_time - p.start_time,
                p.energy
            )
        } else {
            "unknown".into()
