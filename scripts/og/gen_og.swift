...
// for mixr.
//
//   swift scripts/og/gen_og.swift                  # writes site/src/assets/og/hero.png
...
// rounded body with the shell-prompt `> mixr` wordmark, mixr's teal
// accent on the `>`, near-white on the name. A tagline sits in the
// right column, with a family chip at the bottom. 1200x630 (Twitter's
// recommended card size — also fits Open Graph spec).
...
let WIDTH: Int = 1200
let HEIGHT: Int = 630
let TAGLINE = "Beatport streaming + AI mixing in your terminal."
...
func render() -> Data? {
    ...
    // as the app icon (rounded charcoal body, teal prompt accent).
    // We place it on the left and use the right column for the
    // tagline and family chip.
    ...
    // Card wordmark — `> mixr`, centered inside the rounded card.
    ...
    cardWordmark.append(NSAttributedString(string: "mixr", attributes: [
        .font: cardFont,
    ...
    head.append(NSAttributedString(string: "mixr", attributes: [
        .font: headFont,
        .foregroundColor: textColor,
        .paragraphStyle: leftPara,
        .kern: -headFontSize * 0.04,
