...
- **Family settings UI convention.** mixr's Settings screen follows the shared
  family idiom (also used by mnml). Section headers
...
**Tmnl/blit integration removed (2026-06-23):** Mixr no longer
runs as a tmnl native pane via the blit protocol. Mnml's
2026-06-22 cleanup retired the whole tmnl-protocol approach;
this is mixr's mirror cleanup. Removed:

- `src/tui/blit.rs` (656 lines) — the entire `--blit <socket>` client
- `src/tui/theme.rs` (194 lines) — was built specifically for the
  blit-host palette sync; mixr now renders standalone via crossterm
- `--blit` / `--no-native-promote` CLI flags + the auto-promote-
  to-tmnl-tab code path (~80 lines) in `main.rs`
- `App::native_mode` field + the layout adjustment that used it
- `tmnl-protocol` Cargo dependency
- All `git clone tmnl-protocol` steps from `.github/workflows/`
  (CI + release.yml) + `.github/build-setup.yml`
- Stale site docs (tmnl family page, tmnl icon image)

Preserved:
- The `~/.config/mnml/current-theme.toml` shared-palette
  convention. Mixr reads it for live theme sync with mnml.
- The settings overlay's family UI convention (sectioned rows,
  `r` reset / `R` reset all / Enter save).

Net diff: 22 files / +65 / -1171. 303 tests pass; clippy clean.

For prior history (the arc that built the tmnl integration) see
